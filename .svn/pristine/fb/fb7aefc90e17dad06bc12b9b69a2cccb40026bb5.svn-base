    ...
    <c:set var="sss" value=""/>
			...
			<c:forEach items="${_CURRENT_USER.roles }" var="role">
			       ...
			       <c:forEach items="${role.modules }" var="module">
			            ...
			            <c:if test="${module.ctype==0 and module.state==1  }">
				              <c:if test="${fn:contains(sss,module.cpermission) eq false }">
				                   <c:set var="sss" value="${sss},${module.cpermission }"/> 

			                      <shiro:hasPermission name="${module.cpermission }">
	<span id="topmenu" onclick="toModule('${module.remark }');">${module.name }</span><span id="tm_separator"></span>
	</shiro:hasPermission>
			               </c:if>   
			            </c:if>
