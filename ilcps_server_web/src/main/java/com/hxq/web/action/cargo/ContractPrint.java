...
import com.hxq.utils.file.PoiUtil;
...
public class ContractPrint{
	...
	public void print(Contract contract,List<ContractProduct> oList,String path, HttpServletResponse response) throws Exception{
		...
		for(int i=0;i<oList.size();i++){
			...
			if(contract.getPrintStyle() == 2){  
				i++;	//读取第二个货物信息
				if(i<oList.size()){
					oProduct = oList.get(i);  // oList.iterator().next();迭代器总是获取第一个
...
//		sheet.removeRowBreak(6);
		...
		for(int p=0;p<pageList.size();p++){
			...
			if(contract.getPrintStyle() == 2 && UtilFuns.isNotEmpty(printMap.get("ProductNo2"))){
				nRow = sheet.createRow(curRow++);
				nRow.setHeightInPoints(96);
